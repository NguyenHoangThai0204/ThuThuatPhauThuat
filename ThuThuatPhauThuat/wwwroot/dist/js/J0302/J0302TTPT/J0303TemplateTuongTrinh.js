...
function renderTemplates(tbody, templates) {
    tbody.innerHTML = "";
    // ❌ ĐÃ XÓA: Reset về trạng thái Thêm mới khi load danh sách mới
...
function updateIdKhoaInFormData(newKhoaId) {
    ...
    if (titleInputEl) {
        ...
        // Không reset titleInputEl.value
    }
...
async function loadTemplatesByKhoa(value) {
    ...
    // ✅ LOGIC MỚI: XỬ LÝ TRẠNG THÁI FORM KHI THAY ĐỔI KHOA
    if (isEdit) {
        // Nếu đang Cập nhật template cũ, chuyển Khoa MỚI -> Phải clear form và chuyển sang Thêm mới
        clearFormData();
    } else {
        // Nếu đang ở trạng thái Thêm mới, KHÔNG clear nội dung, chỉ cập nhật IDKhoa
...
async function loadChiTietThongTin(id) {
    ...
    try {
        ...
        tbody.innerHTML = "";
        if (dichVus.length === 0) {
            // Luôn mở textarea cho nội dung trống
            tbody.innerHTML = `<tr>
...
async function createNewTemplate() {
    ...
    try {
        ...
        // Cập nhật trạng thái và data cho template mới tạo
        isEdit = true;
        updateSaveButtonText();
        ...
        // Gán dữ liệu mới vào dataset.dv và dataset.originalData
...
async function saveTitle(showToast = true) {
    ...
    // Kiểm tra xem tiêu đề đã thay đổi so với dữ liệu gốc chưa
    ...
    try {
        const res = await fetch("/template_tuong_trinh/CapNhat", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            // Chỉ gửi ID và TEN để cập nhật tiêu đề
        ...
        if (newValue !== originalContent) {
            ...
            try {
                const res = await fetch("/template_tuong_trinh/CapNhat", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    // Chỉ gửi ID và NOI DUNG để cập nhật chi tiết
...
function isValidation(checkId = true) {
    ...
    // 3. Kiểm tra ID (Chỉ cần ID khi ở trạng thái Cập nhật)
    if (checkId) {
...
if (btn) btn.addEventListener("click", saveAllEdits);


// --- cancel edit (Xử lý nút Hủy: Revert hoặc Reset) ---
function cancelEdit(showToast = true) {
    if (isEdit) {
        // STATE: Cập nhật (Item Selected) -> Revert to Original
        ...
        // 3. Giữ nguyên trạng thái Cập nhật và hiện nút Hủy
        isEdit = true;
        updateSaveButtonText();
        if (showToast) toastr && toastr.info("Đã khôi phục nội dung về trạng thái ban đầu.");

    } else {
        // STATE: Thêm mới (No Item Selected) -> Full Reset (Clear fields)
...
function initTomSelectKhoa(data) {
    ...
    selectKhoa = new window.TomSelect(".cbTemplateKhoa", {
        ...
        maxItems: 1,
        // Thêm option ID=0 cho placeholder và là giá trị ban đầu
    ...
    // Thiết lập giá trị ban đầu là 0 (không load data)
    selectKhoa.setValue(0);
}

// ✅ HÀM RESET CỨNG (HARD RESET) CHO FORM THÊM MỚI
function clearFormData(showToast = false) {
    ...
    if (titleInputEl) {
        titleInputEl.value = "";
        // Thiết lập metadata cho trạng thái Thêm mới trên Khoa hiện tại
    ...
    const tbody = document.getElementById("dichVuTable");
    if (tbody) {
        // Luôn mở textarea cho trạng thái thêm mới
        tbody.innerHTML = `<tr>
    ...
    isEdit = false;
    updateSaveButtonText(); // Ẩn nút Hủy
...
async function initializeApp() {
    ...
    if (modalEl) {
        modalEl.addEventListener("shown.bs.modal", () => {
            // Luôn clear form khi mở modal để bắt đầu trạng thái sạch
            clearFormData();
        });

        modalEl.addEventListener("hidden.bs.modal", () => {
            // Reset tất cả về trạng thái ban đầu khi đóng modal
            clearFormData();
            if (selectKhoa) selectKhoa.setValue(0);
            goiVuTable.innerHTML = `<tr><td class="text-center text-muted">Vui lòng chọn khoa để xem danh sách</td></tr>`;
            document.getElementById("dichVuTable").innerHTML = `<tr><td class="text-center text-muted">Không có nội dung</td></tr>`;
        });
    }
}
